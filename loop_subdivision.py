...
def subdivision_loop(mesh):
    ...
    # Update calculation for even vertices to handle boundary cases
    for i, vertex in enumerate(vertices):
        if i in edge_bound:
            # Find the two boundary neighbors of the vertex
            boundary_neighbors = [edges[j]
                                  for j in np.where(edges == i)[0] if edge_bound_mask[j]]
            if boundary_neighbors:
                neighbor_vertices = np.array(
                    [vertices[nb].mean(axis=0) for nb in boundary_neighbors])
